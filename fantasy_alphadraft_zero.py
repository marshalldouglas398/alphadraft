...
class AlphaDraftConfig:
    ...
    validation_team_count: int = 0  # 0 = all teams
    ...
    min_lr: float = 1e-5
    mcts_simulations_final: int = 0  # 0 = no ramp
    shortlist_size_final: int = 0  # 0 = no ramp
    opponent_pool_size: int = 5
    opponent_current_weight: float = 0.5
    opponent_heuristic_weight: float = 0.2
    training_league_sizes: list = field(default_factory=list)
    ensemble_checkpoints: list = field(default_factory=list)


POS_TO_CODE = {"QB": 0, "RB": 1, "WR": 2, "TE": 3}
CODE_TO_POS = {0: "QB", 1: "RB", 2: "WR", 3: "TE"}
...
class SeasonPool:
    ...
    position_codes: np.ndarray  # shape (N,) int8, QB=0 RB=1 WR=2 TE=3
    team_codes: np.ndarray  # shape (N,) int16, integer team index
    team_names: list  # unique NFL team names, ordered by code
...
class OpponentPool:
    def __init__(self, capacity: int):
        self.capacity = capacity
        self.checkpoints: Deque[dict] = deque(maxlen=capacity)
...
class EnsemblePolicy(nn.Module):
    ...
    def forward(
        self, context: "torch.Tensor", candidates: "torch.Tensor", mask: "torch.Tensor",
    ) -> Tuple["torch.Tensor", "torch.Tensor"]:
        all_logits = []
        all_values = []
        for m in self.models:
            logits, value = m(context, candidates, mask)
            all_logits.append(logits)
            all_values.append(value)
        return torch.stack(all_logits).mean(0), torch.stack(all_values).mean(0)
...
class DraftEnvironment:
    def __init__(self, season_pool: SeasonPool, config: AlphaDraftConfig):
        ...
        self._pos_counts = np.zeros((config.league_size, 4), dtype=np.int32)
        num_nfl_teams = len(season_pool.team_names)
        self._team_counts = np.zeros((config.league_size, num_nfl_teams), dtype=np.int32)
        self._cached_remaining_by_pos: np.ndarray | None = None
        self._cached_starter_demand: np.ndarray | None = None
    ...
    def clone(self) -> "DraftEnvironment":
        ...
        clone._pos_counts = self._pos_counts.copy()
        clone._team_counts = self._team_counts.copy()
        clone._cached_remaining_by_pos = None
        clone._cached_starter_demand = None
        return clone
    ...
    def roster_counts(self, team_index: int) -> Dict[str, int]:
        c = self._pos_counts[team_index]
        return {"QB": int(c[0]), "RB": int(c[1]), "WR": int(c[2]), "TE": int(c[3])}
    ...
    def legal_actions(self, team_index: int) -> np.ndarray:
        ...
        counts = self._pos_counts[team_index]
        pos_codes = self.pool.position_codes[available_indices]
        caps = np.array([BENCH_SOFT_CAPS["QB"], BENCH_SOFT_CAPS["RB"],
                         BENCH_SOFT_CAPS["WR"], BENCH_SOFT_CAPS["TE"]], dtype=np.int32)
        under_cap = counts[pos_codes] < caps[pos_codes]
        qb_ok = (pos_codes != 0) | (counts[0] < 2)
        te_ok = (pos_codes != 3) | (counts[3] < 2)
        eligible_mask = under_cap & qb_ok & te_ok
        if eligible_mask.any():
            return available_indices[eligible_mask]
        return available_indices.astype(int)

    def _remaining_by_pos_arr(self) -> np.ndarray:
        if self._cached_remaining_by_pos is None:
            codes = self.pool.position_codes[self.available_mask]
            self._cached_remaining_by_pos = np.bincount(codes, minlength=4).astype(np.int32)
        return self._cached_remaining_by_pos

    def remaining_counts_by_position(self) -> Dict[str, int]:
        c = self._remaining_by_pos_arr()
        return {"QB": int(c[0]), "RB": int(c[1]), "WR": int(c[2]), "TE": int(c[3])}

    def _starter_demand_arr(self) -> np.ndarray:
        if self._cached_starter_demand is None:
            starters = np.array([PRIMARY_STARTERS["QB"], PRIMARY_STARTERS["RB"],
                                 PRIMARY_STARTERS["WR"], PRIMARY_STARTERS["TE"]], dtype=np.int32)
            gaps = np.maximum(starters[np.newaxis, :] - self._pos_counts, 0)
            demand = gaps.sum(axis=0).astype(np.float32)
            flex_filled_all = np.minimum(
                np.maximum(self._pos_counts[:, 1] - PRIMARY_STARTERS["RB"], 0)
                + np.maximum(self._pos_counts[:, 2] - PRIMARY_STARTERS["WR"], 0)
                + np.maximum(self._pos_counts[:, 3] - PRIMARY_STARTERS["TE"], 0),
                FLEX_SLOTS,
            )
            open_flex = np.maximum(FLEX_SLOTS - flex_filled_all, 0).sum()
            flex_share = int(math.ceil(open_flex / len(FLEX_POSITIONS)))
            flex_add = np.array([0, flex_share, flex_share, flex_share], dtype=np.float32)
            self._cached_starter_demand = np.maximum(demand + flex_add, 1.0).astype(np.float32)
        return self._cached_starter_demand

    def remaining_starter_demand(self, position: str) -> int:
        return int(self._starter_demand_arr()[POS_TO_CODE[position]])
    ...
    def _shortlist_scores_vectorized(self, team_index: int, indices: np.ndarray) -> np.ndarray:
        pos_codes = self.pool.position_codes[indices]
        z_scores = self.pool.public_score_z[indices]
        need_arr = np.array([self.position_need_multiplier(team_index, p) for p in POS_LIST], dtype=np.float32)
        rem_arr = self._remaining_by_pos_arr().astype(np.float32)
        demand_arr = self._starter_demand_arr()
        scarcity_vals = demand_arr[pos_codes] / np.maximum(rem_arr[pos_codes], 1.0)
        nfl_teams = self.pool.team_codes[indices]
        roster = self.rosters[team_index]
        if roster:
            roster_arr = np.array(roster, dtype=np.intp)
            roster_pos = self.pool.position_codes[roster_arr]
            roster_teams = self.pool.team_codes[roster_arr]
            same_team_matrix = nfl_teams[:, np.newaxis] == roster_teams[np.newaxis, :]
            roster_is_wr_te = (roster_pos == 2) | (roster_pos == 3)
            roster_is_qb = roster_pos == 0
            is_qb = pos_codes == 0
            is_wr_te = (pos_codes == 2) | (pos_codes == 3)
            stack_vals = np.where(
                is_qb, (same_team_matrix & roster_is_wr_te[np.newaxis, :]).sum(axis=1),
                np.where(is_wr_te, (same_team_matrix & roster_is_qb[np.newaxis, :]).sum(axis=1), 0)
            ).astype(np.float32)
            dup_vals = self._team_counts[team_index, nfl_teams].astype(np.float32)
        else:
            stack_vals = np.zeros(len(indices), dtype=np.float32)
            dup_vals = np.zeros(len(indices), dtype=np.float32)
        counts = self._pos_counts[team_index]
        qb_pen = np.where(
            (pos_codes == 0) & (counts[0] >= 1) & (self.current_round() <= 10),
            0.25, 0.0
        )
        return (z_scores
                + 0.55 * np.log1p(np.maximum(need_arr[pos_codes], 0.0))
                + 0.45 * scarcity_vals
                + 0.12 * stack_vals
                - 0.14 * dup_vals
                - qb_pen)
    ...
    def shortlist_score(self, team_index: int, player_idx: int) -> float:
        return float(self._shortlist_scores_vectorized(team_index, np.array([player_idx]))[0])

    def shortlist(self, team_index: int, limit: int) -> np.ndarray:
        legal = self.legal_actions(team_index)
        if legal.size <= limit:
            return legal
        scores = self._shortlist_scores_vectorized(team_index, legal)
        if legal.size > limit:
            top_order = np.argpartition(scores, -limit)[-limit:]
            top_order = top_order[np.argsort(scores[top_order])[::-1]]
            return legal[top_order]
        return legal[np.argsort(scores)[::-1]]

    def stack_signal(self, team_index: int, player_idx: int) -> float:
        pos_code = self.pool.position_codes[player_idx]
        nfl_team = self.pool.team_codes[player_idx]
        roster = self.rosters[team_index]
        if not roster:
            return 0.0
        roster_arr = np.array(roster, dtype=np.intp)
        roster_pos = self.pool.position_codes[roster_arr]
        roster_teams = self.pool.team_codes[roster_arr]
        same_team = roster_teams == nfl_team
        if pos_code == 0:
            return float(((roster_pos == 2) | (roster_pos == 3)) & same_team).sum()
        if pos_code in (2, 3):
            return float((roster_pos == 0) & same_team).sum()
        return 0.0

    def same_team_penalty(self, team_index: int, player_idx: int) -> float:
        return float(self._team_counts[team_index, self.pool.team_codes[player_idx]])
    ...
    def build_context_vector(self, team_index: int) -> np.ndarray:
        ...
        c = self._pos_counts[team_index]
        caps = np.array([BENCH_SOFT_CAPS["QB"], BENCH_SOFT_CAPS["RB"],
                         BENCH_SOFT_CAPS["WR"], BENCH_SOFT_CAPS["TE"]], dtype=np.float32)
        flex_filled_val = min(
            max(int(c[1]) - PRIMARY_STARTERS["RB"], 0)
            + max(int(c[2]) - PRIMARY_STARTERS["WR"], 0)
            + max(int(c[3]) - PRIMARY_STARTERS["TE"], 0),
            FLEX_SLOTS,
        )
        flex_gap = max(FLEX_SLOTS - flex_filled_val, 0)
        other_mask = np.ones(self.config.league_size, dtype=bool)
        other_mask[team_index] = False
        other_counts = self._pos_counts[other_mask].astype(np.float32)
        if other_counts.size == 0:
            other_counts = np.zeros((1, 4), dtype=np.float32)
        avail_pos = self.pool.position_codes[self.available_mask]
        avail_scores = self.pool.public_score[self.available_mask]
        remaining_top = np.zeros(4, dtype=np.float32)
        remaining_frac = np.zeros(4, dtype=np.float32)
        total_remaining = max(avail_pos.size, 1)
        for p in range(4):
            pmask = avail_pos == p
            if pmask.any():
                remaining_top[p] = avail_scores[pmask].max()
            remaining_frac[p] = float(pmask.sum()) / total_remaining
        context = np.array(
            [
                round_norm,
                pick_norm,
                available_frac,
                c[0] / caps[0], c[1] / caps[1], c[2] / caps[2], c[3] / caps[3],
                self.starter_gap(team_index, "QB"),
                self.starter_gap(team_index, "RB"),
                self.starter_gap(team_index, "WR"),
                self.starter_gap(team_index, "TE"),
                float(flex_gap),
                other_counts[:, 0].mean() / caps[0],
                other_counts[:, 1].mean() / caps[1],
                other_counts[:, 2].mean() / caps[2],
                other_counts[:, 3].mean() / caps[3],
                remaining_top[0], remaining_top[1], remaining_top[2], remaining_top[3],
                remaining_frac[0], remaining_frac[1], remaining_frac[2], remaining_frac[3],
            ],
            dtype=np.float32,
        )
        return context
    ...
    def build_candidate_matrix(self, team_index: int, candidate_indices: np.ndarray) -> np.ndarray:
        pos_codes = self.pool.position_codes[candidate_indices]
        c = self._pos_counts[team_index]
        caps = np.array([BENCH_SOFT_CAPS["QB"], BENCH_SOFT_CAPS["RB"],
                         BENCH_SOFT_CAPS["WR"], BENCH_SOFT_CAPS["TE"]], dtype=np.float32)
        need_arr = np.array([self.position_need_multiplier(team_index, p) for p in POS_LIST], dtype=np.float32)
        ...
        demand_arr = self._starter_demand_arr()
        starters = np.array([PRIMARY_STARTERS["QB"], PRIMARY_STARTERS["RB"],
                             PRIMARY_STARTERS["WR"], PRIMARY_STARTERS["TE"]], dtype=np.float32)
        flex_filled_val = min(
            max(int(c[1]) - int(starters[1]), 0)
            + max(int(c[2]) - int(starters[2]), 0)
            + max(int(c[3]) - int(starters[3]), 0),
            FLEX_SLOTS,
        )
        flex_gap = float(max(FLEX_SLOTS - flex_filled_val, 0))
        base_gaps = np.maximum(starters - c.astype(np.float32), 0.0)
        starter_gaps = base_gaps.copy()
        for p in [1, 2, 3]:
            starter_gaps[p] += 0.5 * flex_gap
        nfl_teams = self.pool.team_codes[candidate_indices]
        roster = self.rosters[team_index]
        if roster:
            roster_arr = np.array(roster, dtype=np.intp)
            roster_pos = self.pool.position_codes[roster_arr]
            ...
            stack_vals = np.zeros(len(candidate_indices), dtype=np.float32)
            dup_vals = np.zeros(len(candidate_indices), dtype=np.float32)
        is_flex = np.isin(pos_codes, [1, 2, 3]).astype(np.float32)
        round_norm = self.current_round() / max(self.config.draft_rounds, 1)
        dynamic_matrix = np.column_stack([
            need_arr[pos_codes],
            c[pos_codes].astype(np.float32) / caps[pos_codes],
            starter_gaps[pos_codes],
            is_flex,
            stack_vals,
            dup_vals,
            demand_arr[pos_codes] / np.maximum(rem_arr[pos_codes], 1.0),
            self.pool.public_score_z[candidate_indices],
            np.full(len(candidate_indices), round_norm, dtype=np.float32),
        ]).astype(np.float32)
    ...
    def step(self, player_idx: int) -> None:
        ...
        pidx = int(player_idx)
        self.rosters[team_index].append(pidx)
        self.available_mask[pidx] = False
        self._pos_counts[team_index, self.pool.position_codes[pidx]] += 1
        self._team_counts[team_index, self.pool.team_codes[pidx]] += 1
        self._cached_remaining_by_pos = None
        self._cached_starter_demand = None
        self.draft_history.append({
            "player_idx": pidx,
            "team_index": team_index,
            "overall_pick": int(slot["overall_pick"]),
            "round": int(slot["round"]),
            "pick_in_round": int(slot["pick_in_round"]),
        })
        self.slot_idx += 1
    ...
    def evaluate_terminal(self) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        for _, row in team_results.iterrows():
            ...
            reward_map[team_index] = np.float32(
                self.config.reward_rank_weight * rank_reward + self.config.reward_point_weight * point_reward
            )
    ...
    def draft_board_frame(self) -> pd.DataFrame:
        ...
        for entry in self.draft_history:
            player = self.pool.players.iloc[entry["player_idx"]].to_dict()
            player["fantasy_team"] = f"Team {entry['team_index'] + 1}"
            player["team_index"] = entry["team_index"]
            player["overall_pick"] = entry["overall_pick"]
            player["round"] = entry["round"]
            player["pick_in_round"] = entry["pick_in_round"]
            rows.append(player)
...
def build_season_pools(
    ...
    for season in seasons:
        ...
        position_codes = season_df["position"].map(POS_TO_CODE).to_numpy(dtype=np.int8)
        team_names_list = sorted(season_df["team"].unique().tolist())
        team_to_code = {t: i for i, t in enumerate(team_names_list)}
        team_codes = season_df["team"].map(team_to_code).to_numpy(dtype=np.int16)
...
def interpolate_param(initial: int, final: int, iteration: int, total_iterations: int) -> int:
    if final <= 0:
        return initial
    frac = min(iteration / max(total_iterations, 1), 1.0)
    return int(round(initial + frac * (final - initial)))
...
def blended_policy_prior(
    ...
    heuristic_scores = env._shortlist_scores_vectorized(team_index, candidate_indices).astype(np.float32)
...
def choose_heuristic_opponent_action(
    env: DraftEnvironment,
    team_index: int,
    config: AlphaDraftConfig,
    stochastic: bool,
    ...
    candidate_indices = env.shortlist(team_index, config.shortlist_size)
    heuristic_scores = env._shortlist_scores_vectorized(team_index, candidate_indices)
    if stochastic:
        probs = softmax_numpy(heuristic_scores, temperature=config.opponent_temperature)
        return int(np.random.choice(candidate_indices, p=probs))
    return int(candidate_indices[int(np.argmax(heuristic_scores))])
...
def simulate_search(
    ...
    if current_team != perspective_team:
        opp = opponent_model or model
        action = choose_opponent_action(env, current_team, opp, device, config, stochastic=True)
        env.step(action)
        return simulate_search(node, env, perspective_team, model, device, config, focal_depth, opponent_model=opponent_model)
    ...
    env.step(action)
    value = simulate_search(child, env, perspective_team, model, device, config, focal_depth + 1, opponent_model=opponent_model)
    child.visit_count += 1
...
def run_mcts(
    ...
    for _ in range(config.mcts_simulations):
        env_sim = env.clone()
        simulate_search(root, env_sim, perspective_team, model, device, config, focal_depth=0, opponent_model=opponent_model)
...
def play_self_play_episode(
    ...
    model.eval()
    # League size augmentation
    episode_config = config
    if training and config.training_league_sizes:
        sampled_league_size = random.choice(config.training_league_sizes)
        if sampled_league_size != config.league_size:
            episode_config = replace(config, league_size=sampled_league_size)
        if focal_team_index >= sampled_league_size:
            focal_team_index = random.randint(0, sampled_league_size - 1)
    # Opponent diversity
    opponent_model = model
    use_heuristic = False
    if training and opponent_pool is not None:
        roll = random.random()
        if roll < config.opponent_heuristic_weight:
            use_heuristic = True
        elif roll < config.opponent_heuristic_weight + (1.0 - config.opponent_current_weight - config.opponent_heuristic_weight):
            past_sd = opponent_pool.sample()
            if past_sd is not None and context_dim > 0 and candidate_dim > 0:
                opponent_model = PolicyValueNet(context_dim, candidate_dim, config.hidden_dim, config.dropout).to(device)
                opponent_model.load_state_dict(past_sd)
                opponent_model.eval()
    env = DraftEnvironment(season_pool, episode_config)
    examples: List[TrainingExample] = []

    total_picks = len(env.pick_slots)
    ...
    while not env.is_terminal():
        ...
        current_team = env.current_team()
        pick_num = env.slot_idx + 1
        is_focal = current_team == focal_team_index
        if is_focal:
            candidate_indices, visit_probs, context, candidate_matrix, _ = run_mcts(
                ...
                opponent_model=opponent_model if not use_heuristic else None,
            )
            ...
            if use_heuristic:
                action = choose_heuristic_opponent_action(env, current_team, episode_config, stochastic=training)
            else:
        ...
        pick_times.append(pick_elapsed)
        if pick_num <= 5 or pick_num % 30 == 0 or pick_num == total_picks:
            avg_pick = sum(pick_times) / len(pick_times)
            est_episode = avg_pick * total_picks
            elapsed_total = time.time() - episode_start
            current_round = env.current_round() if not env.is_terminal() else "done"
            log_progress(
                f"  Pick {pick_num}/{total_picks} | round={current_round} "
                f"team={current_team} {'FOCAL' if is_focal else 'opp'} | "
                f"{pick_elapsed:.2f}s | avg_pick={avg_pick:.2f}s | "
                f"est_episode={est_episode:.0f}s | elapsed={elapsed_total:.1f}s"
            )
...
def evaluate_on_seasons(
    ...
    for season, season_pool in sorted(season_pools.items()):
        ...
        if 0 < team_count < config.league_size:
            team_indices = sorted(np.random.choice(
                config.league_size, size=team_count, replace=False
            ).tolist())
        else:
            team_indices = list(range(config.league_size))
...
def fit_alpha_self_play_model(
    ...
    if checkpoint_path is not None:
        ...
        log_progress(f"Warm-started from checkpoint: {checkpoint_path}")
    ...
    if config.use_lr_schedule:
        estimated_batches_per_epoch = max(config.replay_buffer_size, config.episodes_per_iteration * config.draft_rounds) // config.batch_size
        t_max = config.self_play_iterations * config.training_epochs_per_iteration * estimated_batches_per_epoch
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(t_max, 1), eta_min=config.min_lr)
        log_progress(f"LR schedule: CosineAnnealingLR | T_max={t_max} | eta_min={config.min_lr}")
    ...
    for iteration in range(1, config.self_play_iterations + 1):
        effective_mcts = interpolate_param(config.mcts_simulations, config.mcts_simulations_final, iteration, config.self_play_iterations)
        effective_shortlist = interpolate_param(config.shortlist_size, config.shortlist_size_final, iteration, config.self_play_iterations)
        iter_config = replace(config, mcts_simulations=effective_mcts, shortlist_size=effective_shortlist) if (effective_mcts != config.mcts_simulations or effective_shortlist != config.shortlist_size) else config
        log_progress(f"Iteration {iteration}/{config.self_play_iterations} | self-play generation started | mcts={effective_mcts} shortlist={effective_shortlist}")
        ...
        for episode_number in range(1, config.episodes_per_iteration + 1):
            ...
            avg_ep = sum(episode_durations) / len(episode_durations)
            remaining_episodes = (
                (config.episodes_per_iteration - episode_number)
                + (config.self_play_iterations - iteration) * config.episodes_per_iteration
            )
            est_remaining_hrs = (avg_ep * remaining_episodes) / 3600.0
            if episode_number % max(config.log_every_episodes, 1) == 0:
                ...
                log_progress(
                    ...
                    f"ep_time={ep_duration:.1f}s | avg_ep={avg_ep:.1f}s | "
                    f"est_remaining={est_remaining_hrs:.2f}h | "
        ...
        train_metrics = train_on_replay_buffer(model, optimizer, replay_buffer, device, config, iteration=iteration, scheduler=scheduler)
        opponent_pool.add(unwrap_model(model).state_dict())
        train_episode_df = pd.DataFrame(episode_rows)

        run_validation = (
            iteration % config.validation_frequency == 0
            or iteration == config.self_play_iterations
        )
        if run_validation:
            log_progress(f"Iteration {iteration} | validation started")
            validation_df = evaluate_on_seasons(
                valid_pools, model, device, config,
                phase_label=f"validation iter {iteration}",
                team_count=config.validation_team_count,
            )
            ...
            log_progress(f"Iteration {iteration} | skipping validation (frequency={config.validation_frequency})")
            validation_reward = -np.inf
            validation_points = np.nan
            validation_rank = np.nan
        ...
        if not run_validation:
            pass  # no validation this iteration, skip early stopping check
        elif validation_reward > best_validation_reward:
...
def parse_args() -> AlphaDraftConfig:
    ...
    parser.add_argument("--mcts-simulations-final", type=int, default=0, help="Final MCTS sims (0=no ramp)")
    parser.add_argument("--shortlist-size-final", type=int, default=0, help="Final shortlist size (0=no ramp)")
    parser.add_argument("--training-league-sizes", type=int, nargs="*", default=[], help="League sizes for augmentation")
    parser.add_argument("--ensemble-checkpoints", type=Path, nargs="*", default=[], help="Checkpoints for ensemble inference")
    ...
    return AlphaDraftConfig(
        ...
        validation_team_count=args.validation_team_count,
        validation_frequency=args.validation_frequency,
        replay_buffer_size=args.replay_buffer_size,
        mcts_simulations_final=args.mcts_simulations_final,
        shortlist_size_final=args.shortlist_size_final,
        training_league_sizes=args.training_league_sizes,
...
def main() -> None:
    ...
    # Ensemble for test draft if checkpoints provided
    test_model = model
    if config.ensemble_checkpoints:
        ensemble_models = []
        for ckpt_path in config.ensemble_checkpoints:
            m = PolicyValueNet(context_dim, candidate_dim, config.hidden_dim, config.dropout).to(device)
            ckpt = torch.load(ckpt_path, map_location=device, weights_only=True)
            m.load_state_dict(ckpt["model_state_dict"])
            m.eval()
            ensemble_models.append(m)
        ensemble_models.append(unwrap_model(model))
        test_model = EnsemblePolicy(ensemble_models).to(device)
        log_progress(f"Ensemble created with {len(ensemble_models)} models for test draft")
